...
% FIT_POLYANI_WIGNER_NIEMANT_TRIPLETEST Fits multiple TDS spectra using the same kinetic parameters with weighted least squares and parameter scaling
% Inputs:
...
scale_factors = [1e5, 1e4, 1e7, 10^10]; % Scale to roughly 1-10 range
...
objective = @(scaled_params) calculate_error_multiple_weighted_scaled(scaled_params, exp_data, beta, init_tmp, max_tmp, scale_factors);
...
if ~isempty(scaled_hessian) && all(size(scaled_hessian) == [4 4]) && all(~isnan(scaled_hessian(:)))
...
if ~isempty(hessian) && all(size(hessian) == [4 4]) && all(~isnan(hessian(:)))
    ...
    fprintf('Eigenvalues: [%.2e, %.2e, %.2e, %.2e]\n', eigenvals(1), eigenvals(2), eigenvals(3), eigenvals(4));
end
end

function error = calculate_error_multiple_weighted_scaled(scaled_params, exp_data, beta, init_tmp, max_tmp, scale_factors)
