...
[~ ,p25_tmp, p25_rate, ~] = polyani_wigner(5, 300, 30*4184, 0 , 10^13, 0.25,600 );
...
[~ ,p50_tmp, p50_rate, ~] = polyani_wigner(5, 300, 30*4184, 10 , 10^13, 0.5,600 );
...
[~ ,p75_tmp, p75_rate, ~] = polyani_wigner(5, 300, 30*4184, 0 , 10^13, 0.75,600 );
...
[~ ,p0_tmp, p0_rate, ~] = polyani_wigner(5, 300, 30*4184, 0 , 10^13, 1,600 );
...
%7/31 - added debug statements, can only really replicate 

%% 7/30 - 1st order HEX (ACTUAL)  

theta0 = 0.1; % try 0.25, 0.33, 0.5, 0.66
data = MONTECARLO_HEX_JUL25(theta0);
dTheta_dT = -gradient(data(:,2), data(:,1));

figure(2); clf; hold on;

scatter(data(:,1), dTheta_dT, 20, 'filled', 'DisplayName', sprintf('\\theta_0 = %.2f', theta0));
%[~ ,p25_tmp, p25_rate, ~] = polyani_wigner(5, 300, 31.6*4184, 0 , 10^13, 0.3,700 );
%plot(p25_tmp,p25_rate, 'r');
...
xlabel('Temperature (K)');
ylabel('Desorption Rate');
title('TPD Spectrum: 1st Order Desorption on Hexagonal Lattice (Set E)');
legend show; grid on; 
ax = gca;
ax.YAxis.Exponent = 0;  % Removes scientific notation
ax.YAxis.TickLabelFormat = '%.4f';  
%7/30 - Lines up with polyani-wigner at 0 interaction energy (nn and nnn)
%Gets peak temperature right but not the general shape 


%% 7/31 - SET A FIGURE 1 TEST
...
for i = 1:runs
setA_MC = MC_TRI_TEST(0.5);
scatter(setA_MC(:,1), -gradient(setA_MC(:,2), setA_MC(:,1)), 20, 'filled', 'g', 'DisplayName', 'Coverage = 0.50');

end
[~ ,p50_tmp, p50_rate, ~] = polyani_wigner(5, 300, 30*4184, 0 , 10^13, 0.5,600 );
plot(p50_tmp,p50_rate, 'g');

for i = 1:runs
setA_MC = MC_TRI_TEST(0.75);
scatter(setA_MC(:,1), -gradient(setA_MC(:,2), setA_MC(:,1)), 20, 'filled', 'b', 'DisplayName', 'Coverage = 0.75');
...
setA_MC = MC_TRI_TEST(0.5);
scatter(setA_MC(:,1), -gradient(setA_MC(:,2), setA_MC(:,1)), 20, 'filled', 'g', 'DisplayName', 'Coverage = 0.50');
[~ ,p50_tmp, p50_rate, ~] = polyani_wigner(1, 300, 30*4184, 0 , 10^15, 0.5,500 );
plot(p50_tmp,p50_rate, 'g');
