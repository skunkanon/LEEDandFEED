...
eps_a        = 0.4;            % emissivity (polished copper 0.1 to 0.2; oxidized can be much higher), doesnt affect ramp rate though 

spotweldscale = 15; %NEW VARIABLE, 12/28
...
odefun = @(t,y) [ ...
    ...
    ( (I(t) * I_amp_single).^2 .* R_wire_FUNCT(y(1)) ...   % Joule heat, W, R_wire_FUNCT(y(1))
      ...
      -  eps_w * sigma * SA_wire_single * ( y(1).^4 - T_env^4 ) ... % radiation (all wires)
    ) / (heatcap_wire)  ; ...
    % --- crystal node (y2) ---
...
% Crystal heating rate (data): dT/dt [K/s] 
t_IRL = timespan_IRL(:);
T_IRL = tempspan_IRL(:);
...
nexttile(2);
plot(t, conduct_wireFUNCT(T_w) .* (T_w-300) , 'LineWidth', 1.8);
xlabel('time (s)'); ylabel('Watts');
title('Power through one wire');
grid on;
...
%% GRADIENT PLOT 1/2 
 ...
 q_vol_sample = P_wire_sample ./ volume_wire;   % W/m^3, 'internal heat generation' 
...
hold on;
 for k = 1:numProfiles
     ...
     plot(x_mm, T_x - 273, 'LineWidth', 1.6, ...
 ...
 title('Spatial Wire Temperature Profile');
