function [TPD_data, energyStats] = MONTECARLO_HEX2_AUG4(L, theta0, Ed0, eps_nn, eps_nnn, beta, k0, targetCoverage)
...
% Reference: B. Meng and W.H. Weinberg, J. Chem. Phys. 100, 5280 (1994).
    ...
    % (and identify the maximum rate r_max as per algorithm:contentReference[oaicite:7]{index=7})
    [r_max, ~] = computeDesorptionRates();   % custom function to compute r_max (see below)
    
    % Main KMC loop for desorption events
    eventCount = 0;
    while currentN > 0
        % Choose a random occupied site (candidate for desorption)
        % (We will use the acceptance-rejection method with r_max:contentReference[oaicite:8]{index=8})
        ...
        Ed = Ed0 - eps_nn*nn_count - eps_nnn*nnn_count;        % effective desorption barrier for this site:contentReference[oaicite:9]{index=9}
        r_idx = k0 * exp(-Ed / (kB * T_history(eventCount+1))); % desorption rate for this site at current T
        ...
        if rand() < P
            ...
            occ(idx) = false;
            currentN = currentN - 1;
            % Update simulation time by time increment Δt = 1 / (sum of all rates):contentReference[oaicite:10]{index=10}
            totalRate = computeTotalRate();    % sum of rates over all occupied sites at current state
            if totalRate > 0
                dt = 1 / totalRate;
            else
                % If totalRate is zero (e.g., extremely low T), set a large dt to jump in time
            ...
            [r_max, ~] = computeDesorptionRates(idx);  
            % Optionally, relax the surface via diffusion to equilibrium after desorption:contentReference[oaicite:11]{index=11}:contentReference[oaicite:12]{index=12}
...
function [r_max, rates] = computeDesorptionRates(modifiedIdx)
    % computeDesorptionRates - Compute desorption rate for each occupied site and return max.
    % If modifiedIdx is provided, only update rates for sites affected by that index (for efficiency).
    % For simplicity, this implementation recomputes all rates each time.
    ...
    for ii = 1:length(occ_linear_indices)
        ...
        r_site = k0 * exp(-E_site / (kB * T_history(eventCount+1)));
        if r_site > r_max
            r_max = r_site;
        end
        % (In a full implementation, one could store r_site in 'rates' array if needed)
    end
end

% Helper function to compute total desorption rate (sum over all occupied sites)
function totalR = computeTotalRate()
    totalR = 0;
    occ_linear_indices = find(occ);
    for ii = 1:length(occ_linear_indices)
        site = occ_linear_indices(ii);
